import { window as vswindow,workspace  } from "vscode"
...
export class Logger {
  ...
  public log = (message: string) => {
    ...
    if (!output)
      vswindow.showErrorMessage("Error getting the output channel from VS Code")

    output.appendLine(
      `${this._date.toLocaleDateString()} ${this._hour} INFO ${message}`
    )
  }
  ...
  public error = (err: NodeJS.ErrnoException) => {
    ...
    output.appendLine(
      `${this._date.toLocaleDateString()} ${this._hour} ERROR ${err.message}`
    )
  }
